...
public class ProductControllerImplTest {
    ...
    public void getById() throws Exception {
        ...
        Mockito.when(productService.findById(ArgumentMatchers.anyInt())).thenReturn(java.util.Optional.of(ProductBuilder.getEntity()));
    ...
    public void update() throws Exception {
        Mockito.when(productMapper.asEntity(ArgumentMatchers.any())).thenReturn(ProductBuilder.getEntity());
        Mockito.when(productService.update(ArgumentMatchers.any(), ArgumentMatchers.anyInt())).thenReturn(ProductBuilder.getEntity());
        ...
        mockMvc.perform(
                        MockMvcRequestBuilders.put(ENDPOINT_URL + "/1")
                                ...
                                .content(CustomUtils.asJsonString(ProductBuilder.getDTO())))
                .andExpect(MockMvcResultMatchers.status().isOk());
        Mockito.verify(productService, Mockito.times(1)).update(ArgumentMatchers.any(Product.class), ArgumentMatchers.anyInt());
        Mockito.verifyNoMoreInteractions(productService);
    }

    @Test
    public void delete() throws Exception {
        Mockito.doNothing().when(productService).deleteById(ArgumentMatchers.anyInt());
        mockMvc.perform(
